...
for i, method in enumerate(sorted_methods):
    if method in aggregated_data1:
        data1 = [score*100 for comp_scores in aggregated_data1[method].values() for score in comp_scores]
        plt.boxplot(data1, positions=[positions[i] - 0.15], patch_artist=True, boxprops=dict(facecolor=colors[0]), widths=0.3, medianprops=dict(color='black'), flierprops=outlier_props)
    if method in aggregated_data2:
        data2 = [score*100 for comp_scores in aggregated_data2[method].values() for score in comp_scores]
        plt.boxplot(data2, positions=[positions[i] + 0.15], patch_artist=True, boxprops=dict(facecolor=colors[1]), widths=0.3, medianprops=dict(color='black'), flierprops=outlier_props)
...
# plt.xlabel("Methods")
plt.ylabel("Remaining preys percentage", fontsize=16)
...
save_path = 'plots'
plt.savefig(os.path.join(save_path, 'remaining_preys_comparison.png'), dpi=300)
plt.savefig(os.path.join(save_path, 'remaining_preys_comparison.svg'), dpi=300)
plt.savefig(os.path.join(save_path, 'remaining_preys_comparison.pdf'), dpi=300)
