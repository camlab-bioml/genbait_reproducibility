        ...
        for file_name in files:
            ...
            for number_of_components in components_range:
                ...
                if len(subset_indices) > number_of_components:
                    ...
                    results_ga[num_features][number_of_components].append(np.min(np.diag(cos_sim_matrix)))
        ...
        for number_of_components in components_range:
            ...
            for column in range(column_start, column_end):
                ...
                if len(subset_data) >= number_of_components:
                    ...
                    basis_matrix_subset_reordered = basis_matrix_subset[:, col_ind]
                    cos_sim_matrix = cosine_similarity(basis_matrix_original.T, basis_matrix_subset_reordered.T)
                    results_random[bait_length][number_of_components].append(np.min(np.diag(cos_sim_matrix)))
                    ...
                    if method in methods_df:
                        ...
                        for number_of_components in components_range:
                            if len(subset_indices) >= number_of_components:
                                ...
                                basis_matrix_subset_reordered = basis_matrix_subset[:, col_ind]
                                cos_sim_matrix = cosine_similarity(basis_matrix_original.T, basis_matrix_subset_reordered.T)
                                results_ml[method][entity_count][number_of_components].append(np.min(np.diag(cos_sim_matrix)))
    
    return results_ml


def plot_nmf_cos_scores_min(df_norm, components_range, ga_path , ml_path , random_path , save_path):
    os.makedirs(save_path, exist_ok=True)

    ga_pickle_path = os.path.join(save_path, 'nmf_scores_cos_ga_min.pkl')
    random_pickle_path = os.path.join(save_path, 'nmf_scores_cos_random_min.pkl')
    ml_pickle_path = os.path.join(save_path, 'nmf_scores_cos_ml_min.pkl')
    ...
    ga_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_cos_ga_min.pkl'), 'rb'))
    random_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_cos_random_min.pkl'), 'rb'))
    ml_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_cos_ml_min.pkl'), 'rb'))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf cos scores vs. each method min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf cos scores vs. each method min.svg'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf cos scores vs. each method min.pdf'), dpi=300)
    plt.clf()
    ...
    # plt.ylim(0,1)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf cos scores vs. each method (sorted) min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf cos scores vs. each method (sorted) min.svg'), dpi=300)
    plt.clf()
    ...
    plt.savefig(os.path.join(save_path, 'nmf cos score vs. number of baits min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf cos score vs. number of baits min.svg'), dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf cos score vs. number of baits_mean with shades min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf cos score vs. number of baits_mean with shades min.svg'), dpi=300)
    plt.clf()
