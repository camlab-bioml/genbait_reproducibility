...
def process_ga_results(df_norm, original_probs_dict, cluster_numbers, ga_path, seed=4):
    random.seed(seed)
    results_ga = {}
    directory = ga_path
    feature_groups = {}
    ...
    for file_name in os.listdir(directory):
        if file_name.endswith('.csv'):
            ...
            feature_groups.setdefault(num_features, []).append(file_name)
    ...
    for num_features, files in feature_groups.items():
        ...
        for file_name in files:
            ...
            for cluster_number in cluster_numbers:
                ...
                results_ga[num_features][cluster_number].append(mean_corr)

    return results_ga


def process_random_baseline(df_norm, original_probs_dict, cluster_numbers, random_path, seed=4):
    random.seed(seed)
    ...
    for bait_length in range(30, 81):
        for column in range((bait_length - 30) * 10, (bait_length - 29) * 10):
            ...
            for cluster_number in cluster_numbers:
                ...
                subset_probs = get_gmm_soft_assignments(subset_data_np, cluster_number, seed)
                mean_corr = average_cluster_correlation(original_probs, subset_probs)
                results_random[bait_length][cluster_number].append(mean_corr)

    return results_random


def process_ml_results(df_norm, original_probs_dict, cluster_numbers, ml_path, seed=4):
    random.seed(seed)
                ...
                for entity_count in range(30, 81):
                    if method in methods_df:
                        ...
                        for cluster_number in cluster_numbers:
                            ...
                            subset_probs = get_gmm_soft_assignments(subset_data_np, cluster_number, seed)
                            mean_corr = average_cluster_correlation(original_probs, subset_probs)
                            results_ml[method][entity_count][cluster_number].append(mean_corr)
...
def analyze_for_seed(df_norm, cluster_numbers, seed, ga_path, ml_path, random_path):
    ...
    ga_results = process_ga_results(df_norm, original_probs_dict, cluster_numbers, ga_path, seed)
    random_results = process_random_baseline(df_norm, original_probs_dict, cluster_numbers, random_path, seed)
    ml_results = process_ml_results(df_norm, original_probs_dict, cluster_numbers, ml_path, seed)
...
def plot_gmm_correlation(df_norm, cluster_numbers, ga_path, ml_path, random_path, save_path, seeds=range(10)):
    results_file=f'{save_path}gmm_results.pkl'
    # Check if the pickle file exists
    if not os.path.exists(results_file):
        run_parallel_analysis(df_norm, seeds, results_file, ga_path, ml_path, random_path)
    ...
    # Setup variables
    method_names = ['GA', 'Random'] + list(results['ML'].keys())  # Combined method names, ML prefixed

    # Aggregate scores for each method across all resolutions
    aggregated_scores = {}
    for method in method_names:
        ...
        aggregated_scores[method] = [item for sublist in scores for item in sublist]  # Flatten list of lists

    # Calculate median scores for sorting
    median_scores = {method: np.median(scores) for method, scores in aggregated_scores.items()}
    
    # Sort methods by median score in descending order
    sorted_methods = sorted(median_scores, key=median_scores.get, reverse=True)

    # Generate boxplot data in sorted order and clean method names
    boxplot_data = [aggregated_scores[method] for method in sorted_methods]
    cleaned_method_names = [method.replace('ML_', '') for method in sorted_methods]  # Clean method names for plotting
    ...
    medianprops = dict(color="orange")  # Set median color to white for contrast
    ...
    plt.xlabel('Method')
    plt.ylabel('Aggregated Mean Correlation Values')
    plt.title('Aggregated Mean Correlation Values by Method')
    plt.ylim(0, 1)
    plt.tight_layout()
    plt.savefig(f'{save_path}GMM mean correlation values vs. each method (sorted).png', dpi=300)
    plt.savefig(f'{save_path}GMM mean correlation values vs. each method (sorted).svg', dpi=300)
    plt.clf()
    ...
    for cluster_number_index, cluster_number in enumerate(cluster_numbers):
        group_center = cluster_number_index * spacing_factor * len(method_names) * single_width

        for method_index, method in enumerate(method_names):
            if method in ['GA', 'Random']:
                mean_correlation = [score for num_features in results[method].values() for score in num_features.get(cluster_number, [])]
            else:  # ML method
                mean_correlation = [score for num_features in results['ML'][method].values() for score in num_features.get(cluster_number, [])]

            position = group_center + offset[method_index]

            plt.boxplot(mean_correlation, positions=[position], widths=single_width*0.9, patch_artist=True,
                        boxprops=dict(facecolor=colors[method_index % len(colors)]))

        tick_positions.append(group_center)

    plt.xticks(tick_positions, [str(cluster_number) for cluster_number in cluster_numbers])
    plt.xlabel('Number of clusters')
    plt.ylabel('mean correlation values')
    plt.title('Mean correlation values by method and cluster numbers')
    ...
    plt.tight_layout()
    plt.savefig(f'{save_path}GMM mean correlation values vs. number of clusters.png', dpi=300)
    plt.savefig(f'{save_path}GMM mean correlation values vs. number of clusters.svg', dpi=300)
    plt.clf()


    # Setup for plot
    n_cluster_numbers = len(cluster_numbers)
    color_map = cm.get_cmap('Blues', n_cluster_numbers + 1)  # '+1' to avoid the lightest color
    cluster_numbers_colors = [color_map(i) for i in range(1, n_cluster_numbers + 1)]
    method_names = ['GA', 'Random'] + ['ML_' + method for method in ml_names]  # Combined method names
    color_dict = dict(zip(cluster_numbers, cluster_numbers_colors))

    # Prepare data for plotting
    plot_data = {method: {cluster_number: [] for cluster_number in cluster_numbers} for method in method_names}

    # Extract data for each method and resolution
    for method_group, methods in results.items():
        if method_group == 'ML':
            for method, method_data in methods.items():
                for num_features, cluster_number_data in method_data.items():
                    for cluster_number, mean_correlation in cluster_number_data.items():
                        plot_data['ML_' + method][cluster_number].extend(mean_correlation)
        else:  # GA or Random
            for num_features, cluster_number_data in methods.items():
                for cluster_number, mean_correlation in cluster_number_data.items():
                    plot_data[method_group][cluster_number].extend(mean_correlation)
    ...
    for i, res in enumerate(cluster_numbers):
        ...
        for method_index, method in enumerate(method_names):
            ...
            ax.boxplot(plot_data[method][res], positions=[pos], widths=0.12, patch_artist=True,
                    boxprops=dict(facecolor=color_dict[res]), flierprops=flierprops)

    # Customize the axes
    ax.set_xticks(positions)
    ax.set_xticklabels([method.replace('ML_', '') for method in method_names], rotation=90)
    ax.set_ylabel('GMM mean correlation values')
    ax.set_title('GMM mean correlation values vs. each method')

    # Create a legend for the resolutions
    from matplotlib.patches import Patch
    legend_elements = [Patch(facecolor=color_dict[cluster_number], label=cluster_number) for cluster_number in cluster_numbers]
    ax.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(1, 1.05), title="Number of clusters")

    plt.tight_layout()
    plt.savefig(f'{save_path}GMM mean correlation values vs. each method.png', dpi=300)
    plt.savefig(f'{save_path}GMM mean correlation values vs. each method.svg', dpi=300)
    plt.clf()
    ...
    for method_group, methods in results.items():
        for method, method_data in (methods.items() if method_group == 'ML' else [(method_group, methods)]):
            for num_features, cluster_number_data in method_data.items():
                for cluster_number, mean_correlation in cluster_number_data.items():
                    all_values.extend(mean_correlation)
    ...
    for idx, method in enumerate(method_names):
        ...
        for num_features in feature_numbers:
            ...
            mean_correlation = [score for cluster_number in cluster_numbers for score in method_data[num_features].get(cluster_number, [])]
            boxplot_data.append(mean_correlation)
    ...
    plt.savefig(f'{save_path}GMM mean correlation values vs. number of baits.png', dpi=300)
    plt.savefig(f'{save_path}GMM mean correlation values vs. number of baits.svg', dpi=300)
    plt.clf()
    ...
    # Set plot parameters
    ax.set_xlabel('Number of Baits')
    ax.set_ylabel('Aggregated mean correlation scores')
    ax.set_title('Mean Correlation Scores by Number of Baits for Each Method')
    ax.legend(loc='upper left', bbox_to_anchor=(1, 1.05))
    ax.set_xlim(left=min(feature_numbers), right=max(feature_numbers))
    ax.set_ylim(0, 1)
    plt.tight_layout()
    plt.savefig(f'{save_path}GMM mean correlation values values vs. number of baits_mean shaded.png', dpi=300)
    plt.savefig(f'{save_path}GMM mean correlation values values vs. number of baits_mean shaded.svg', dpi=300)
    plt.clf()
