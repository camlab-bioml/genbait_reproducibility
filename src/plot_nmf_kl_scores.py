        ...
        for file_name in files:
            ...
            for number_of_components in components_range:
                ...
                if len(subset_indices) > number_of_components:
                            ...
                            results_ga[num_features][number_of_components].append(np.mean(np.diag(kl_div_matrix)))
        ...
        for number_of_components in components_range:
            ...
            for column in range(column_start, column_end):
                ...
                if len(subset_data) >= number_of_components:
                    ...
                    results_random[bait_length][number_of_components].append(np.mean(np.diag(kl_div_matrix)))
                        ...
                        for number_of_components in components_range:
                            if len(subset_indices) >= number_of_components:
                                ...
                                for i in range(number_of_components):
                                    for j in range(number_of_components):
                                        kl_div_matrix[i, j] = kl_divergence(basis_matrix_original_normalized[:, i], basis_matrix_subset_reordered_normalized[:, j])
                                results_ml[method][entity_count][number_of_components].append(np.mean(np.diag(kl_div_matrix)))
    
    return results_ml


def plot_nmf_kl_scores(df_norm, components_range, ga_path, ml_path, random_path, save_path):
    os.makedirs(save_path, exist_ok=True)

    ga_pickle_path = os.path.join(save_path, 'nmf_scores_kl_ga.pkl')
    random_pickle_path = os.path.join(save_path, 'nmf_scores_kl_random.pkl')
    ml_pickle_path = os.path.join(save_path, 'nmf_scores_kl_ml.pkl')
    ...
    ga_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_kl_ga.pkl'), 'rb'))
    random_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_kl_random.pkl'), 'rb'))
    ml_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_kl_ml.pkl'), 'rb'))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf kl scores vs. each method.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl scores vs. each method.svg'), dpi=300)
    plt.clf()
    ...
    plt.ylim(0,16)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf kl scores vs. each method (sorted).png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl scores vs. each method (sorted).svg'), dpi=300)
    plt.clf()
    ...
    plt.savefig(os.path.join(save_path, 'nmf kl score vs. number of baits.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl score vs. number of baits.svg'), dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf kl score vs. number of baits_mean with shades.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl score vs. number of baits_mean with shades.svg'), dpi=300)
    plt.clf()
