...
def plot_nmf_max_kl_combined(dataset_paths, component_ranges, save_path, output_prefix):
    """
    ...
    def load_and_aggregate_data(dataset_path, components_range):
        ...
        ga_data = pickle.load(open(os.path.join(dataset_path, 'nmf_scores_kl_ga_min.pkl'), 'rb'))
        random_data = pickle.load(open(os.path.join(dataset_path, 'nmf_scores_kl_random_min.pkl'), 'rb'))
        ml_data = pickle.load(open(os.path.join(dataset_path, 'nmf_scores_kl_ml_min.pkl'), 'rb'))
    ...
    plt.ylabel("Max NMF KL divergence score", fontsize=18)
    plt.legend(handles=[
