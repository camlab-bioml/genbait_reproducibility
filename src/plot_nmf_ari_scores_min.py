...
def calculate_component_preservation(df_norm, df_subset, n_components, random_state=46):
    """
    Calculates the preservation of preys for each component in the subset compared to the original dataset.
    Returns the minimum preservation value across all components.
    """
    ...
    # Convert basis matrices to DataFrames for indexing
    basis_original_df = pd.DataFrame(basis_matrix_original, index=df_norm.columns)
    basis_subset_reordered_df = pd.DataFrame(basis_matrix_subset_reordered, index=df_subset.columns)

    # Step 3: Identify common indices
    common_indices = basis_original_df.index.intersection(basis_subset_reordered_df.index)

    # Filter rows for common indices
    common_basis_original = basis_original_df.loc[common_indices].to_numpy()
    common_basis_subset = basis_subset_reordered_df.loc[common_indices].to_numpy()

    # Step 4: Calculate prey preservation for each component
    component_preservations = []
    for component_idx in range(n_components):
        # Identify preys assigned to this component in the original data
        original_preys = np.where(np.argmax(common_basis_original, axis=1) == component_idx)[0]

        if len(original_preys) == 0:  # No preys assigned to this component in the original
            component_preservations.append(1.0)  # Full preservation if nothing to lose
            continue

        # Check how many of those preys are still assigned to the same component in the subset
        subset_preys = np.where(np.argmax(common_basis_subset, axis=1) == component_idx)[0]
    ...
    # Step 5: Return the minimum preservation value
    min_preservation = min(component_preservations)
    return min_preservation




ml_names = ['chi_2', 'f_classif', 'mutual_info_classif', 'lasso', 'ridge', 'elastic_net', 'rf', 'gbm', 'xgb', 'nn']
...
def process_ga_results(df_norm, components_range, ga_path):
    ...
    the minimum ARI values across components for each number of components.
    """
    results_ga = {}
    directory = ga_path
    ...
    for num_features, files in feature_groups.items():
        ...
        for file_name in files:
            ...
            for number_of_components in components_range:
                ...
                # Call the updated calculate_ari function to get the minimum ARI
                min_ari_score = calculate_component_preservation(df_norm, subset_data, number_of_components)
                results_ga[num_features][number_of_components].append(min_ari_score)
...
def process_random_baseline(df_norm, components_range, random_path):
    ...
    Calculates the minimum ARI values across components for each random bait subset.
    """
...
def process_ml_results(df_norm, components_range, ml_path):
    ...
    Calculates the minimum ARI values across components for each ML-selected bait subset.
    """
                ...
                for entity_count in range(30, 81):
                    if method in methods_df:
                        ...
                        for number_of_components in components_range:
                            # Call the updated calculate_ari function to get the minimum ARI
                            min_ari_score = calculate_component_preservation(df_norm, subset_data, number_of_components)
                            results_ml[method][entity_count][number_of_components].append(min_ari_score)
    
    return results_ml



def plot_nmf_ari_scores_min(df_norm, components_range, ga_path , ml_path , random_path , save_path):
    os.makedirs(save_path, exist_ok=True)

    ga_pickle_path = os.path.join(save_path, 'nmf_scores_ari_ga_min.pkl')
    random_pickle_path = os.path.join(save_path, 'nmf_scores_ari_random_min.pkl')
    ml_pickle_path = os.path.join(save_path, 'nmf_scores_ari_ml_min.pkl')
    ...
    ga_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_ari_ga_min.pkl'), 'rb'))
    random_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_ari_random_min.pkl'), 'rb'))
    ml_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_ari_ml_min.pkl'), 'rb'))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf ari values vs. each method min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf ari values vs. each method min.svg'), dpi=300)
    plt.clf()
    ...
    # plt.ylim(0,1)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf ari values vs. each method (sorted) min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf ari values vs. each method (sorted) min.svg'), dpi=300)
    plt.clf()
    ...
    plt.savefig(os.path.join(save_path, 'nmf ari values vs. number of baits min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf ari values vs. number of baits min.svg'), dpi=300)
    plt.close()
...
# Define methods and initialize storage for aggregated data
    ...
    # ax.set_ylim(0, 1)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf ari score vs. number of baits_mean with shades min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf ari score vs. number of baits_mean with shades min.svg'), dpi=300)
    plt.clf()
