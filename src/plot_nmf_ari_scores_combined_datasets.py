...
def plot_nmf_ari_combined(dataset_paths, component_ranges, save_path, output_prefix):
    """
    ...
    def load_and_aggregate_data(dataset_path, components_range):
        ...
        ga_data = pickle.load(open(os.path.join(dataset_path, 'nmf_ari_scores_ga.pkl'), 'rb'))
        random_data = pickle.load(open(os.path.join(dataset_path, 'nmf_ari_scores_random.pkl'), 'rb'))
        ml_data = pickle.load(open(os.path.join(dataset_path, 'nmf_ari_scores_ml.pkl'), 'rb'))
    ...
    plt.ylabel("NMF ARI", fontsize=18)
    plt.legend(handles=[
