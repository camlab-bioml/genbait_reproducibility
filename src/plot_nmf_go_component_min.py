...
def calculate_go_score_per_component(df_norm, df_subset, n_components, random_state=46):
    ...
    def go_analysis_single_query(prey_names, timeout=10):
        """
        Perform a single GO analysis query with a timeout.
        If the query exceeds the timeout, it is skipped.
        """
        try:
            time.sleep(1)
            # Perform the API call with a timeout
            go_df = gp.profile(organism='hsapiens', query=prey_names)
            go_df = go_df[go_df['source'] == 'GO:CC']
            return set(go_df['native'])
        except requests.exceptions.Timeout:
            print(f"Timeout for query: {prey_names}")
            return set()
        except Exception as e:
            print(f"Error processing GO analysis for {prey_names}: {e}")
            return set()

    def process_go_analysis_parallel(grouped_df, timeout=10):
        """
        Perform GO analysis in parallel, skipping queries that exceed the timeout.
        """
        top_native_terms = {}
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_label = {
                executor.submit(go_analysis_single_query, list(group.index), timeout): label 
                for label, group in grouped_df
            }
    ...
    mean_jaccard_index = np.min(list(jaccard_indices.values()))
...
def plot_nmf_go_scores_min(df_norm, components_range, ga_path , ml_path , random_path , save_path):
    os.makedirs(save_path, exist_ok=True)

    ga_pickle_path = os.path.join(save_path, 'nmf_scores_go_components_ga_min.pkl')
    random_pickle_path = os.path.join(save_path, 'nmf_scores_go_components_random_min.pkl')
    ml_pickle_path = os.path.join(save_path, 'nmf_scores_go_components_ml_min.pkl')

    # Check if the pickle files exist
    if not all([
                os.path.exists(ga_pickle_path),
                 os.path.exists(random_pickle_path),
                 os.path.exists(ml_pickle_path)
                   ]):
    ...
    ga_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_go_components_ga_min.pkl'), 'rb'))
    random_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_go_components_random_min.pkl'), 'rb'))
    ml_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_go_components_ml_min.pkl'), 'rb'))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf go components scores values vs. each method min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf go components scores values vs. each method min.svg'), dpi=300)
    plt.clf()
    ...
    plt.ylim(0,1)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf go components scores vs. each method (sorted) min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf go components scores vs. each method (sorted) min.svg'), dpi=300)
    plt.clf()
...
# Extract scores for GA and XGB methods
    ...
    plt.savefig(os.path.join(save_path, 'nmf go components scores vs. number of baits min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf go components scores vs. number of baits min.svg'), dpi=300)
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf go score vs. number of baits_mean with shades min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf go score vs. number of baits_mean with shades min.svg'), dpi=300)
    plt.clf()
