...
def plot_nmf_min_purity_score_combined(dataset_paths, component_ranges, save_path, output_prefix):
    """
    ...
    def load_and_aggregate_data(dataset_path, components_range):
        ...
        ga_data = pickle.load(open(os.path.join(dataset_path, 'nmf_ari_scores_ga_min.pkl'), 'rb'))
        random_data = pickle.load(open(os.path.join(dataset_path, 'nmf_ari_scores_random_min.pkl'), 'rb'))
        ml_data = pickle.load(open(os.path.join(dataset_path, 'nmf_ari_scores_ml_min.pkl'), 'rb'))
    ...
    plt.ylabel("Min NMF purity score", fontsize=18)
    plt.legend(handles=[
