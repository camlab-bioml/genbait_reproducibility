        ...
        for file_name in files:
            ...
            for number_of_components in components_range:
                ...
                if len(subset_indices) > number_of_components:
                    ...
                    # Correct normalization along columns
                    ...
                    # Initialize an empty matrix to store the KL divergence values
                    kl_div_matrix = np.zeros((number_of_components, number_of_components))

                    # Calculate the KL divergence between each pair of components
                    ...
                    results_ga[num_features][number_of_components].append(np.max(np.diag(kl_div_matrix)))
        ...
        for number_of_components in components_range:
            ...
            for column in range(column_start, column_end):
                ...
                if len(subset_data) >= number_of_components:
                    ...
                    results_random[bait_length][number_of_components].append(np.max(np.diag(kl_div_matrix)))
                        ...
                        for number_of_components in components_range:
                            if len(subset_indices) >= number_of_components:
                                ...
                                for i in range(number_of_components):
                                    for j in range(number_of_components):
                                        kl_div_matrix[i, j] = kl_divergence(basis_matrix_original_normalized[:, i], basis_matrix_subset_reordered_normalized[:, j])
                                results_ml[method][entity_count][number_of_components].append(np.max(np.diag(kl_div_matrix)))
    
    return results_ml


def plot_nmf_kl_scores_min(df_norm, components_range, ga_path , ml_path , random_path , save_path):
    os.makedirs(save_path, exist_ok=True)

    ga_pickle_path = os.path.join(save_path, 'nmf_scores_kl_ga_min.pkl')
    random_pickle_path = os.path.join(save_path, 'nmf_scores_kl_random_min.pkl')
    ml_pickle_path = os.path.join(save_path, 'nmf_scores_kl_ml_min.pkl')
    ...
    ga_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_kl_ga_min.pkl'), 'rb'))
    random_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_kl_random_min.pkl'), 'rb'))
    ml_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_kl_ml_min.pkl'), 'rb'))
    ...
    box_plot_width = 1.5  # Adjust width to avoid overlap within groups

    spacing_factor = 2.5  # Adjust this factor to increase/decrease the spacing between groups
    method_spacing = 2  # Adjust this factor to increase/decrease the spacing within groups
    ...
    for idx, method in enumerate(method_names):
        ...
        for comp_idx, comp in enumerate(components_range):
            ...
            pos = positions[idx] + comp_idx * method_spacing - (len(components_range) * method_spacing / 2)
            plt.boxplot(
                all_data[method][comp], 
                positions=[pos], 
                widths=box_plot_width, 
                patch_artist=True, 
                boxprops=dict(facecolor=component_colors[comp_idx], color=component_colors[comp_idx]), 
    ...
    plt.xlabel("Methods")
    plt.ylabel("NMF KL Divergence Diagonal Mean Values")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf kl scores vs. each method min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl scores vs. each method min.svg'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl scores vs. each method min.pdf'), dpi=300)
    plt.clf()
    ...
    sorted_methods = sorted(method_names, key=lambda x: medians[x], reverse=False)
    ...
    plt.xlabel("Methods")
    plt.ylabel("NMF KL Divergence Mean Values")
    # plt.title("Comparison of Methods across all number of components", fontsize=20)
    # plt.ylim(0,16)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf kl scores vs. each method (sorted) min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl scores vs. each method (sorted) min.svg'), dpi=300)
    plt.clf()
    ...
    plt.suptitle('NMF KL Divergence Mean Values by Number of Baits for Each Method')
    plt.savefig(os.path.join(save_path, 'nmf kl score vs. number of baits min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl score vs. number of baits min.pdf'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl score vs. number of baits min.svg'), dpi=300)
    plt.close()
    ...
    ax.set_ylabel('Aggregated NMF KL Scores')
    ax.set_title('Aggregated NMF KL Scores by Number of Baits for Each Method')
    ax.legend(loc='upper left', bbox_to_anchor=(1, 1.05))
    ax.set_xlim(left=min(bait_numbers), right=max(bait_numbers))
    # ax.set_ylim(0, 1)

    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf kl score vs. number of baits_mean with shades min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl score vs. number of baits_mean with shades min.pdf'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf kl score vs. number of baits_mean with shades min.svg'), dpi=300)
    plt.clf()
