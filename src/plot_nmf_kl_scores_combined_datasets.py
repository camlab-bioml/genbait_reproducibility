...
def load_and_aggregate_data(dataset_path, components_range):
    ...
    ga_data = pickle.load(open(os.path.join(dataset_path, 'nmf_scores_kl_ga.pkl'), 'rb'))
    random_data = pickle.load(open(os.path.join(dataset_path, 'nmf_scores_kl_random.pkl'), 'rb'))
    ml_data = pickle.load(open(os.path.join(dataset_path, 'nmf_scores_kl_ml.pkl'), 'rb'))
...
sorted_methods = sorted(median_values, key=median_values.get)
...
# plt.xlabel("Methods")
plt.ylabel("NMF mean KL divergence score", fontsize=16)
...
save_path = 'plots'
plt.savefig(os.path.join(save_path, 'nmf_scores_kl_comparison_ordered.png'), dpi=300)
plt.savefig(os.path.join(save_path, 'nmf_scores_kl_comparison_ordered.svg'), dpi=300)
plt.savefig(os.path.join(save_path, 'nmf_scores_kl_comparison_ordered.pdf'), dpi=300)
plt.clf()