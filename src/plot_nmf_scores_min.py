        ...
        for file_name in files:
            ...
            for number_of_components in components_range:
                ...
                if len(subset_indices) > number_of_components:
                    ...
                    results_ga[num_features][number_of_components].append(np.min(np.diag(corr_matrix)))
        ...
        for number_of_components in components_range:
            ...
            for column in range(column_start, column_end):
                ...
                if len(subset_data) >= number_of_components:
                    ...
                    # Calculating correlation matrix
                    corr_matrix = np.corrcoef(basis_matrix_original, basis_matrix_subset_reordered, rowvar=False)[:number_of_components, number_of_components:]
                    results_random[bait_length][number_of_components].append(np.min(np.diag(corr_matrix)))
                    ...
                    if method in methods_df:
                        ...
                        for number_of_components in components_range:
                            if len(subset_indices) >= number_of_components:
                                ...
                                # Calculating correlation matrix
                                corr_matrix = np.corrcoef(basis_matrix_original, basis_matrix_subset_reordered, rowvar=False)[:number_of_components, number_of_components:]
                                results_ml[method][entity_count][number_of_components].append(np.min(np.diag(corr_matrix)))
...
def plot_nmf_scores_min(df_norm, components_range, ga_path , ml_path , random_path , save_path):
    os.makedirs(save_path, exist_ok=True)

    ga_pickle_path = os.path.join(save_path, 'nmf_scores_ga_min.pkl')
    random_pickle_path = os.path.join(save_path, 'nmf_scores_random_min.pkl')
    ml_pickle_path = os.path.join(save_path, 'nmf_scores_ml_min.pkl')
    ...
    ga_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_ga_min.pkl'), 'rb'))
    random_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_random_min.pkl'), 'rb'))
    ml_data = pickle.load(open(os.path.join(save_path, 'nmf_scores_ml_min.pkl'), 'rb'))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf scores vs. each method min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf scores vs. each method min.svg'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf scores vs. each method min.pdf'), dpi=300)
    plt.clf()
    ...
    # plt.ylim(0, 1)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf scores vs. each method min (sorted).png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf scores vs. each method min (sorted).svg'), dpi=300)
    plt.clf()
    ...
    plt.savefig(os.path.join(save_path, 'nmf score vs. number of baits min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf score vs. number of baits min.svg'), dpi=300)
    plt.clf()
    ...
    # ax.set_ylim(0, 1)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'nmf score vs. number of baits_mean with shades min.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'nmf score vs. number of baits_mean with shades min.svg'), dpi=300)
    plt.clf()
