    ...
    for num_features, files in feature_groups.items():
        ...
        for file_name in files:
            ...
            for number_of_components in components_range:
                ...
                results_ga[num_features][number_of_components].append(prey_count/df_norm.shape[1])
    ...
    for bait_length in range(30, 81):
        ...
        for number_of_components in components_range:
            ...
            for column in range(column_start, column_end):
                ...
                prey_count = np.count_nonzero(np.count_nonzero(subset_data, axis=0))
                results_random[bait_length][number_of_components].append(prey_count/df_norm.shape[1])
                ...
                for entity_count in range(30, 81):
                    if method in methods_df:
                        ...
                        for number_of_components in components_range:
                            subset_data = original_data[subset_indices, :]
                            prey_count = np.count_nonzero(np.count_nonzero(subset_data, axis=0))
                            results_ml[method][entity_count][number_of_components].append(prey_count/df_norm.shape[1])
    
    return results_ml


def plot_remaining_preys(df_norm, components_range, ga_path, ml_path, random_path, save_path):
    os.makedirs(save_path, exist_ok=True)

    ga_pickle_path = os.path.join(save_path, 'remaining_preys_ga.pkl')
    random_pickle_path = os.path.join(save_path, 'remaining_preys_random.pkl')
    ml_pickle_path = os.path.join(save_path, 'remaining_preys_ml.pkl')
    ...
    ga_data = pickle.load(open(os.path.join(save_path, 'remaining_preys_ga.pkl'), 'rb'))
    random_data = pickle.load(open(os.path.join(save_path, 'remaining_preys_random.pkl'), 'rb'))
    ml_data = pickle.load(open(os.path.join(save_path, 'remaining_preys_ml.pkl'), 'rb'))
    ...
    plt.xlabel("Methods")
    plt.ylabel("Percentage of remaining preys")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'remaining preys vs. each method.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'remaining preys vs. each method.svg'), dpi=300)
    plt.clf()
    ...
    plt.tight_layout()
    plt.ylim(0, 1)
    plt.savefig(os.path.join(save_path, 'remaining preys vs. each method (sorted).png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'remaining preys vs. each method (sorted).svg'), dpi=300)
    plt.clf()
    ...
    plt.suptitle('Percentage of remaining preys by Number of Baits for Each Method')
    plt.savefig(os.path.join(save_path, 'remaining preys vs. number of baits.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'remaining preys vs. number of baits.svg'), dpi=300)
    plt.close()
    ...
    ax.set_ylabel('Aggregated NMF remaining preys ')
    ax.set_title('Aggregated NMF remaining preys by Number of Baits for Each Method')
    ax.legend(loc='upper left', bbox_to_anchor=(1, 1.05))
    ax.set_xlim(left=min(bait_numbers), right=max(bait_numbers))
    ax.set_ylim(0, 1)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'remaining preys vs. number of baits_mean with shades.png'), dpi=300)
    plt.savefig(os.path.join(save_path, 'remaining preys vs. number of baits_mean with shades.svg'), dpi=300)
    plt.clf()
